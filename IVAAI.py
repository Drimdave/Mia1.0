...
def record_audio(ask=""):
    with sr.Microphone() as source:  # microphone as source
        ...
        print("I have heard you what you said")
        voice_data = ''
        try:
            voice_data = r.recognize_google(audio)  # convert audio to text
        except sr.UnknownValueError:  # error: recognizer does not understand
            engine_speak('I did not get that, please repeat')
...
def engine_speak(audio_string):
    ...
    tts = gTTS(text=audio_string, lang='en-NG')  # text to speech(voice)
...
def respond(voice_data):
    ...
    if there_exists(["who are you"]):
        engine_speak("I am a virtual assistant ready to help you with your everyday tasks " + person_obj.name)

    if there_exists(["who made you"]):
        engine_speak("I was created by Master David right in his laptop ")
    ...
    if there_exists(['volume up']):
        engine_speak('volume up sir')
        pyautogui.hotkey('volumeup')

    if there_exists(['volume down']):
        engine_speak('volume downn sir')
        pyautogui.hotkey('volumedown')

    if there_exists(["exit", "quit", "goodbye"]):
        engine_speak("bye and have a lovely day")
        exit()
...
while (1):
    voice_data = record_audio("I'm listening to you")  # get the voice input
    print("Done")
    print("Q:", voice_data)
